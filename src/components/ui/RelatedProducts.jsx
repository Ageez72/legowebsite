...
export default function RelatedProducts({ items }) {
    ...
    async function fetchRelatedProducts() {
        const res = await axios.get(`${BASE_API}${endpoints.products.list}&lang=EN&id=${encodeURIComponent(items.join(','))}&token=${Cookies.get("legoToken")}`, {});
        return res;
    }
    ...
    return (
        <>
            <div className={`${data?.data?.items?.length > 0 ? "grid grid-cols-2 sm:grid-cols-2 md:grid-cols-2 lg:grid-cols-2 xl:grid-cols-4 gap-4" : ""} products-page-listing related-products`}>
                {
                    isLoading && (
                        <VerticalLoader />
                    )
